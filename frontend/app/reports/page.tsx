...
export default function ReportingEngine() {
  ...
  if (loading) return <div className="state-loading">Initializing Engine...</div>;
          ...
          <div className="border border-border bg-white text-black min-h-[600px] flex flex-col">
            ...
            <div className="p-8 flex-1 font-serif text-[13px] leading-relaxed">
              ...
              <div className="space-y-4 text-black">
                <p>This document constitutes an official report generated by the Chennai Regional Pattern Analysis (CRPA) system. The data contained herein is aggregated from the Centralized Incident Database (CID).</p>
                
                {selectedTemplate.id === 'T1' && (
                  <p>In accordance with monthly indexing protocols, the overall crime frequency has been modelled against previous regional baselines. Current trajectory analysis reveals minor fluctuations in property-related indices across high-risk zones, with continuous monitoring established.</p>
                )}
                {selectedTemplate.id === 'T2' && (
                  <p>This executive incident summary outlines critical variables extracted from the field entries. Active investigations are currently leveraging cross-referenced offender records and localized hotspot data to pinpoint operative patterns.</p>
                )}
                {selectedTemplate.id === 'T3' && (
                  <p>A comprehensive review of personnel response times, case clearance rates, and resource allocation across respective stations has been compiled. Field operatives show a 15% increase in closure rates for mid-tier risk incidents this quarter.</p>
                )}
                {selectedTemplate.id === 'T4' && (
                  <p>Geospatial clustering tools indicate an acute shift in incident densities. Priority zones (Risk Level 7+) are observing concentrated activity between 18:00 and 02:00 hours, warranting immediate patrol re-routing optimizations.</p>
                )}

                <div className="p-4 bg-gray-50 border border-gray-200 mt-6 italic text-black">
